...
if [[ $boots == 1 ]]; then
 ...
 lsblk -f
  echo ""
read -p "Укажите boot раздел ( например sda2, nvme0n1p2 ): " bootd
mount /dev/$bootd /mnt
cd /mnt
...
do
 ...
 lsblk -f
  ...
  echo "Создайте 4 раздела: sda1 , type EFI system       - boot"
  echo "                    sda2 , type linux 82 solaris - swap"
  echo "                    sda3 , type linux 83         - root"
  echo "                    sda4 , type linux 83         - home"
  echo "       дважды подумайте об их размерах! "
...
echo ""
lsblk -f
read -p "Укажите ROOT раздел(ex. sda3, nvme0n1p3):" roott
 mkfs.btrfs /dev/$roott
 mount /dev/$roott /mnt/
 clear
echo ""
lsblk -f
read -p "Укажите HOME раздел(ex. sda4, nvme0n1p4):" home
 mkfs.btrfs /dev/$home
...
do
 ...
 lsblk -f
  echo ""
  echo "Сейчас будет провдиться разметка диска."
  echo "Создайте 4 раздела: sda1 с флагом boot тип linux 83;"
  echo "                    sda2 , type linux 82 solaris - swap"
  echo "                    sda3 , type linux 83         - root"
  echo "                    sda4 , type linux 83         - home"
  echo "       дважды подумайте об их размерах! "
